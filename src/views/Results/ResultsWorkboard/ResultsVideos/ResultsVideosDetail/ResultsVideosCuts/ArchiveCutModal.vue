<template lang="pug">
	extends /src/components/Modal/ModalTemplate
	block header
		| {{ $t('Suppression d\'un extrait') }}
	block content
		span.text
			| {{ $t('Confirmez-vous la suppression de l\'extrait ?') }}
	block footer
		div
		div
			button.button(@click="confirm")
				| {{ $t('Supprimer') }}
			button.button.secondary(@click="cancel")
				| {{ $t('Fermer') }}
</template>

<script lang="ts">
import Modal from '@/components/Modal/Modal';
import { Component, Prop } from 'vue-property-decorator';

@Component({})
export default class ArchiveCutModal extends Modal {
	@Prop() protected readonly id!: number;
	@Prop() protected readonly title!: string;

	protected readonly keepalive: boolean = true;
	protected readonly modalId: string = 'archive-modal';

	protected hidden: boolean = true;

	protected confirm(): void {
		this.$emit('archive');
		this.hide();
	}

	protected cancel(): void {
		this.hide();
	}
}
</script>

<style lang="scss">
#archive-modal {
	.content {
		text-align: left;
	}
}
</style>
